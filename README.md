...
3. `novel_probe_status`
4. `novel_query_candidates`
5. 根据候选结果调用 `novel_download`、`novel_download_source_book` 或 `novel_download_cached_result`
6. `novel_download_status`

`novel_refresh_sources` 只是把书源加入后台健康探测队列，不会等待所有探测完成；需要确认进度时，用 `novel_probe_status` 查看当前队列、活跃探测和分页健康摘要。

如果你已经从候选结果里知道要下载的 `source_id + book_url`，不必让 LLM 反复调用高层 `novel_download` 试错，可以直接用 `novel_download_source_book` 指定源下载。若已有 `search_id`，可以先用 `novel_read_search_results` 按 `limit/offset` 分页查看缓存搜索结果，再用 `novel_download_cached_result` 按 `result_index` 继续下载。

这样做是为了让搜索结果、候选列表和探测状态留在插件侧缓存里，LLM 只拿小摘要和稳定 ID，避免高层重试把相同搜索、候选和失败摘要反复塞进上下文造成 token 浪费。
...
3. 调 `novel_download` 或 `novel_auto`
...
- `novel_query_candidates`: 按书名只查询候选下载源和排序结果，不启动下载；可配合 `limit/offset` 分页查看下一批候选
- `novel_inspect_source_book`: 针对指定 `source_id + book_url` 查询预检和正文抽样结果，不创建任务
- `novel_download`: 按书名自动搜书、筛选候选源并启动下载任务
- `novel_download_source_book`: 指定 `source_id + book_url` 启动下载，适合从候选源中手动选定一个源
- `novel_read_search_results`: 按 `search_id` 分页查看缓存搜索结果，避免重复发起同一次搜索
- `novel_download_cached_result`: 按 `search_id + result_index` 从缓存搜索结果继续下载
- `novel_download_status`: 查询下载任务的状态、进度和输出信息

下面这些更底层的能力仍主要用于人工诊断或调试；其中“搜书缓存查看与按结果索引下载”“指定 `source_id + book_url` 的直连下载”已经以受控函数工具形式暴露给 LLM，方便从已有候选继续操作，而不是重新触发高层搜索：

- 网页预览与页面结构分析
- 手工 regex 下载与手工组装
- 底层任务列表与调试恢复

其余低层能力仍更适合由插件内部自动处理，或者由人类命令触发，而不是让 LLM 在上下文里自由组合。

当前最关键的一组能力是：

- 书源管理：`novel_import_sources`、`novel_list_sources`、`novel_get_source_detail`、`novel_refresh_sources`、`novel_remove_source`
- 健康探测：`novel_probe_status`
- 只读查询：`novel_query_candidates`、`novel_inspect_source_book`、`novel_read_search_results`
- 自动化下载：`novel_download`、`novel_download_source_book`、`novel_download_cached_result`
- 状态追踪：`novel_download_status`

`novel_download` 会把“搜书 -> 候选排序 -> 目录预检 -> 失败回退 -> 建任务”收敛成一次确定性工具调用，并额外返回：
...
`novel_query_candidates` 则会把“搜书 -> 候选整理 -> 下载能力过滤 -> 排序”收敛成一次只读查询，适合在真正下载前先看：
...
如果第一页候选不够，可以继续传 `limit/offset` 分页查看下一批候选；如果已经拿到 `search_id`，后续也可以转用 `novel_read_search_results` 读取缓存里的原始搜索结果。

`novel_inspect_source_book` 会在不创建任务的前提下执行：

- 目录预检
- 正文抽样

适合确认“这个源对这本书到底能不能稳定下”，同时避免让 LLM 或用户手动重复试错。
...
`novel_probe_status` 用来接住上一步的后台探测过程，可以查看：
...
这适合在刚导入或刚刷新大量书源后确认“探测是否还在跑”，而不是让 LLM 误以为刷新工具超时或需要反复刷新。

`novel_download_source_book` 适合在 `novel_query_candidates` 已经给出明确候选后使用。只要候选里有 `source_id` 和 `book_url`，就可以指定这个源创建普通下载任务，然后继续用 `novel_download_status` 查进度。
...
同样的缓存延续路径也可以给 LLM 使用：`novel_read_search_results` 用 `search_id + limit/offset` 分页读取缓存结果，`novel_download_cached_result` 用 `search_id + result_index` 从缓存结果启动下载。这样可以从同一批搜索结果继续换页、换源、重试下载，而不是再次调用高层 `novel_download` 重新搜索并重复消耗上下文。
...
- `novel_probe_status` 返回后台探测进度和分页健康摘要，不等待整批探测完成
- `novel_query_candidates` 返回分页候选摘要和 `search_id`，后续可继续按缓存读取
- `novel_download` 默认返回单次高层编排摘要；候选与尝试过多时写本地报告文件并返回 `report_path`
- `novel_download_source_book` 和 `novel_download_cached_result` 复用已知候选或缓存搜索结果，避免为了换源再次触发高层搜索
- 调试路径里的 `novel_search_books` 会额外把搜索结果保存到本地缓存，后续可通过 `search_id` 分页查看或直接下载
- `novel_import_clean_rules` 会把净化规则仓库保存到本地，后续下载时自动应用
- `novel_download_status` 在批量模式下只返回摘要；窗口过大时写本地报告文件
- 调试路径里的 `novel_fetch_preview` 会限制最大预览字符数，避免把整页 HTML 直接塞回聊天平台
...
- 能分页的接口尽量分页
- 必须保留全量结果时，写本地报告文件
- 已经有 `search_id`、`source_id`、`book_url` 或 `result_index` 时，优先从缓存或指定源继续操作，不把高层自动下载当成翻页和换源重试机制
